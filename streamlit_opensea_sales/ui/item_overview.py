...
def render_item_overview(
    ...
    if guide_open:
        ...
        render_section_guide_panel("""<p><b>ITEM ANALYTICS</b> Explore the full observed OpenSea trading history, current supply and market position of a single item.</p>

<p><b>SELECT ITEM</b> Choose any tracked item to load its sales history, market metrics, pricing data and participants.</p>

<p><b>WALLET FILTER</b> Select a wallet to focus on its activity. In CHART view, that wallet's BUY and SELL transactions are highlighted while other trades remain visible for context. In TABLE view, only transactions where the selected wallet was the buyer or seller are shown. Select ALL WALLETS to restore the full history.</p>

<p><b>USD PRICE</b> Switch monetary values between GUN and USD. Historical USD values use the GUN/USD price recorded at the time of each transaction where available.</p>

<p><b>TREND LINE</b> Shows the prepared price trend across the full visible completed-trade history of the selected item.</p>

<p><b>ITEM CARD</b> RANK is the item's Market Strength Rank. SUPPLY shows current on-chain supply, while SUPPLY RANK compares scarcity across the current tracked supply universe. MARKET POSITION shows Volume Rank and Liquidity Rank. MARKET METRICS summarize trading volume, average trading activity and transaction count. PRICING shows average, minimum, maximum and last sale price. PARTICIPANTS counts unique buyers, sellers and wallets.</p>

<p><b>CHART</b> Each point represents an observed completed GUN or WGUN transaction. Hover over a point to inspect the transaction date, sale values, Buyer and Seller. Click a point to pin the same trade tooltip. While it is pinned, click the Buyer or Seller name to open that trader's Profile Card next to the trade details. GUN sales represent direct purchases of listed items: an item was listed at a price and bought. WGUN sales represent accepted offers: a buyer proposed a price and the seller accepted the offer, completing the sale.</p>

<p><b>TABLE</b> Shows the sales history from newest to oldest. Seller and Buyer are shown as trader profile names or stable NoName fallbacks; click either name to open the same Trader Profile Card. GunzScan opens the blockchain transaction, while OpenSea opens the item on OpenSea.</p>

<p><b>TRADER PROFILE CARD</b> Shows the trader's public identity, OpenSea profile link, copyable wallet address and ranked Trading Stats for Earned, Invested, Sold, Trades, ROI and Win Rate.</p>

<p><b>MOBILE VIEW</b> On mobile, ITEM ANALYTICS displays the Item Card and CHART only. The CHART/TABLE view selector is not shown on mobile.</p>

<p class="trader-guide-note"><b>SUPPLY DATA</b> Current supply data is provided by GUNZscope. Supply Rank and the rest of the analytics are calculated by OTG Analytics.</p>

<p class="trader-guide-note">Market and participant metrics are based on observed OpenSea activity and may not represent activity outside the tracked marketplace data.</p>""", trusted_html=True)
