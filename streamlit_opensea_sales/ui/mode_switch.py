...
def render_mode_switch() -> Literal['item', 'market', 'top_items', 'trader', 'ecosystem', 'feedback']:
    ...
    if current_mode not in ('item', 'market', 'top_items', 'trader', 'ecosystem', 'feedback'):
        current_mode = 'item'
    ...
    if current_mode == 'feedback':
        ...
        if source_mode not in ('item', 'market', 'top_items', 'trader', 'ecosystem'):
            source_mode = 'unknown'
    ...
    ecosystem_active = 'active' if current_mode == 'ecosystem' else ''
    st.markdown(f'''<style>
...
.otg-nav-dropdown a{{height:34px;display:flex;align-items:center;padding:0 10px;box-sizing:border-box;border:none;background:transparent;color:#A7ACB3!important;font-size:10px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;text-decoration:none!important;position:relative;transition:color 180ms cubic-bezier(.2,.7,.2,1),background-color 180ms cubic-bezier(.2,.7,.2,1),text-shadow 180ms cubic-bezier(.2,.7,.2,1)}}.otg-nav-dropdown a::before{{content:'';position:absolute;left:0;top:0;width:2px;height:100%;background:#FF003A;transform:scaleY(0);transform-origin:center;opacity:0;transition:opacity 190ms cubic-bezier(.2,.7,.2,1),transform 190ms cubic-bezier(.2,.7,.2,1)}}.otg-nav-dropdown a:hover{{color:#FFFFFF!important;background:rgba(255,255,255,.055);border:none;text-decoration:none!important;padding-left:10px;text-shadow:0 0 12px rgba(255,255,255,.08)}}.otg-nav-dropdown a:hover::before{{opacity:.88;transform:scaleY(1)}}.otg-nav-dropdown a.active{{color:#FFFFFF!important;background:rgba(255,255,255,.080);border:none;text-decoration:none!important;text-shadow:0 0 14px rgba(255,255,255,.10)}}
...
@media(max-width:768px){{.otg-top-nav{{justify-content:flex-end;gap:6px;min-height:32px;min-width:0;max-width:100%;margin-bottom:8px;overflow:visible}}.otg-top-nav-control{{height:30px;flex:0 0 auto;min-width:0;padding-left:8px;padding-right:8px;font-size:9px;letter-spacing:.6px}}.otg-nav-analytics,.otg-nav-analytics>summary{{width:auto;flex:0 0 auto}}.otg-nav-dropdown{{width:176px;max-width:calc(100vw - 24px)}}.otg-nav-dropdown a{{min-height:34px;height:34px}}}}
@media(max-width:480px){{.otg-top-nav{{gap:2px}}.otg-top-nav-control{{padding-left:4px;padding-right:4px;font-size:8px;letter-spacing:.35px}}.otg-nav-chevron{{margin-left:4px}}}}
@media(prefers-reduced-motion:reduce){{.otg-top-nav-control,.otg-top-nav-control::after,.otg-nav-chevron,.otg-nav-dropdown,.otg-nav-dropdown a,.otg-nav-dropdown a::before{{transition:none}}.otg-top-nav-control:hover{{transform:none}}}}
 </style><nav class="otg-top-nav" aria-label="Primary navigation"><details class="otg-nav-analytics"><summary class="otg-top-nav-control {'active' if analytics_active else ''}">ANALYTICS<span class="otg-nav-chevron" aria-hidden="true"></span></summary><div class="otg-nav-dropdown"><a class="{active['item']}" href="/?mode=item">ITEM</a><a class="{active['market']}" href="/?mode=market">MARKET</a><a class="{active['top_items']}" href="/?mode=top_items">TOP ITEMS</a><a class="{active['top_traders']}" href="/?mode=top_traders">TOP TRADERS</a></div></details><a class="otg-top-nav-control {ecosystem_active}" href="/?mode=ecosystem">ECOSYSTEM</a><a class="otg-top-nav-control" href="/?mode=roadmap">ROADMAP</a><a class="otg-top-nav-control {feedback_active}" href="{feedback_href}">FEEDBACK</a><span class="otg-top-nav-control otg-nav-disabled otg-nav-login" aria-disabled="true" title="Coming soon">LOG IN</span></nav>''', unsafe_allow_html=True)
    return current_mode
