...
var tip=null,pinned=false,activePointKey=null,card=null,boundPlot=null,lastPointer=null,
    timer=null,observer=null,dead=false,observerTimer=null,
    nativePlotClickHandler=null,plotlyHoverHandler=null,plotlyUnhoverHandler=null,plotlyClickHandler=null;
...
function capturePointer(e){if(Number.isFinite(e.clientX)&&Number.isFinite(e.clientY))lastPointer={x:e.clientX,y:e.clientY}}
...
function bindPlot(plot){if(!isCurrentItemPlot(plot))return;if(boundPlot===plot)return;if(boundPlot)unbindPlot();boundPlot=plot;nativePlotClickHandler=capturePointer;boundPlot.addEventListener('pointermove',nativePlotClickHandler);boundPlot.addEventListener('pointerdown',nativePlotClickHandler);plotlyHoverHandler=function(e){var p=e.points&&e.points[0];if(!p||!p.data.meta||p.data.meta.otg_point_kind!=='trade'||pinned)return;activePointKey=pointKey(p);var a=resolvePointAnchor(boundPlot,p);renderPoint(p,a.x,a.y)};plotlyUnhoverHandler=function(){if(!pinned)closePinnedTooltip()};plotlyClickHandler=function(e){var p=e.points&&e.points[0];if(!p||!p.data.meta||p.data.meta.otg_point_kind!=='trade')return;var k=pointKey(p);if(activePointKey===k&&tip){pinned=true;return}closeTraderCard();pinned=true;activePointKey=k;var a=resolvePointAnchor(boundPlot,p);renderPoint(p,a.x,a.y)};boundPlot.on('plotly_hover',plotlyHoverHandler);boundPlot.on('plotly_unhover',plotlyUnhoverHandler);boundPlot.on('plotly_click',plotlyClickHandler)}
function unbindPlot(){if(!boundPlot)return;boundPlot.removeEventListener('pointermove',nativePlotClickHandler);boundPlot.removeEventListener('pointerdown',nativePlotClickHandler);if(typeof boundPlot.removeListener==='function'){boundPlot.removeListener('plotly_hover',plotlyHoverHandler);boundPlot.removeListener('plotly_unhover',plotlyUnhoverHandler);boundPlot.removeListener('plotly_click',plotlyClickHandler)}boundPlot=null;nativePlotClickHandler=null;plotlyHoverHandler=null;plotlyUnhoverHandler=null;plotlyClickHandler=null}
...
var state={destroy:function(){dead=true;if(timer)w.clearTimeout(timer);if(observerTimer)w.clearTimeout(observerTimer);timer=null;observerTimer=null;if(observer){observer.disconnect();observer=null}unbindPlot();d.removeEventListener('click',outside);d.removeEventListener('keydown',keys);closeAll();delete w[key]}};
