...
def render_trader_search(records: list[dict[str, str]], selected_wallet: str | None = None, selected_display_name: str | None = None, initial_query: str = "", key: str = "trader_search") -> dict[str, Any] | None:
    ...
    if not isinstance(value, dict) or value.get("action") not in {"select", "clear"} or not isinstance(value.get("event_id"), str):
        return None
    if value["action"] == "select" and not isinstance(value.get("wallet"), str):
        return None
    return value
