...
def _trader_profile_card_styles(metric_icons: dict[str, str | None]) -> str:
    icon_rules = "".join(
        f'.trader-profile-stat-icon--{metric.lower().replace(" ", "-")}{{background-image:url("{src}")}}'
...
.trader-profile-content{{min-width:0;height:var(--trader-profile-square);max-height:var(--trader-profile-square);position:relative;display:flex;flex-direction:column;}}
...
def render_trader_table(rows: list[dict[str, Any]]) -> None:
        ...
        for column in columns:
            ...
            if column in {"Image", "Trader"}:
                ...
                if column == "Image":
                    ...
                    name_link = f'<a class="trader-table-link" href="https://opensea.io/{wallet_html}" target="_blank" rel="noopener noreferrer">{display}{verified}</a>'
                    value = f'<span>{name_link}{secondary}</span>'
    ...
    css = f"""<style>.trader-table-scroll{{overflow-x:auto;width:100%;margin:16px 0}}.trader-table{{width:100%;min-width:1120px;border-collapse:collapse;background:#000;border:1px solid #FF003A;font-family:'Space Mono',monospace;font-size:12px}}.trader-table thead{{background:#0a0a0a;border-bottom:2px solid #FF003A}}.trader-table th{{color:#FF003A;padding:10px 8px;text-align:left;text-transform:uppercase;letter-spacing:.5px;font-size:11px;white-space:nowrap}}.trader-table td{{color:#FFF;padding:8px;text-align:left;font-size:11px;border-bottom:1px solid rgba(255,255,255,.04);white-space:nowrap}}.trader-table td.image{{padding:4px}}.trader-table td.rank-value{{color:#FF003A}}.trader-table tbody tr:hover{{background:#0a0a0a}}.trader-table tbody tr.trader-row-selected{{background:rgba(255,0,58,.07);box-shadow:inset 3px 0 0 #FF003A}}.trader-table .earned-value{{color:{EARNED_COLOR};font-weight:700}}.trader-table .invested-value{{color:{INVESTED_COLOR};font-weight:700}}.trader-table .sold-value{{color:{SOLD_COLOR};font-weight:700}}.trader-table .trader-avatar-small{{border:0;border-radius:0;overflow:visible;background-size:contain}}.trader-image-profile-trigger{{display:inline-flex;align-items:center;gap:8px;position:relative;cursor:pointer}}.trader-image-profile-trigger::after{{content:"";position:absolute;left:48px;top:0;width:16px;height:100%}}.trader-image-profile-trigger .trader-profile-card{{left:64px;top:0;bottom:auto;pointer-events:auto}}.trader-table-link{{color:inherit!important;text-decoration:none!important;font-weight:700}}.trader-table-link:hover{{color:#FF003A!important;font-weight:700}}.trader-avatar{{display:inline-block;flex:0 0 auto;background-image:var(--trader-remote-avatar,none),var(--trader-fallback-avatar);background-size:cover;background-position:center;background-repeat:no-repeat;border-radius:50%;background-color:#111;border:1px solid #FF003A}}.trader-avatar-small{{width:48px;height:48px}}.trader-avatar-large{{width:44px;height:44px}}.trader-profile-secondary{{display:block;color:#C8C8CD;font-size:10px;font-weight:400}}.trader-profile-card{{--trader-profile-square:350px;box-sizing:border-box;border:1px solid #303035;background:#050505;display:none;position:absolute;z-index:1000;left:0;bottom:calc(100% + 8px);top:auto;width:min(980px,calc(100vw - 40px));white-space:normal;padding:14px;color:#FFF;line-height:1.35;box-shadow:0 8px 24px #000}}.trader-profile-card::after{{display:none}}.trader-profile-card::before{{content:"";position:absolute;left:0;right:0;height:8px;bottom:-8px}}.trader-table tbody tr:nth-last-child(-n+6) .trader-profile-card{{top:auto;bottom:calc(100% + 8px)}}.trader-image-profile-trigger:hover .trader-profile-card,.trader-image-profile-trigger:focus-within .trader-profile-card{{display:block}}.trader-profile-card-grid{{display:grid;position:relative;z-index:1;grid-template-columns:var(--trader-profile-square) minmax(0,1fr);height:var(--trader-profile-square);gap:16px;align-items:stretch}}.trader-profile-avatar{{width:var(--trader-profile-square);height:var(--trader-profile-square);aspect-ratio:1/1;background:#080808;position:relative;overflow:hidden;border:1px solid #303035;box-sizing:border-box}}.trader-profile-card .trader-avatar-large{{display:block;width:100%;height:100%;aspect-ratio:1/1;border-radius:0;border:0;background-size:contain;background-position:center;background-repeat:no-repeat}}.trader-profile-avatar::before{{content:"OFF\\A THE\\A GRID";white-space:pre;color:#7b7b82;font-size:9px;line-height:1.05;letter-spacing:2px;position:absolute;left:10px;top:10px;z-index:1;pointer-events:none}}.trader-profile-avatar::after{{content:"TRADERS\\A BUILD\\A DIFFERENT";white-space:pre;color:#7b7b82;font-size:8px;line-height:1.05;letter-spacing:1px;position:absolute;right:10px;bottom:10px;text-align:right;z-index:1;pointer-events:none}}.trader-profile-content{{min-width:0;height:var(--trader-profile-square);max-height:var(--trader-profile-square);position:relative}}.trader-profile-content::before{{content:"TRADER\\A PROFILE";white-space:pre;position:absolute;right:0;top:-2px;color:#77777d;font-size:9px;line-height:1.1;letter-spacing:2px;text-align:right}}.trader-profile-identity{{display:flex;align-items:center;gap:10px;margin-bottom:10px;padding-right:92px}}.trader-profile-identity strong{{display:block;color:#FFF;font-size:32px;line-height:1.05;overflow-wrap:anywhere}}.trader-profile-card a{{color:#FF003A;margin:4px 0;text-decoration:none}}.trader-opensea-link{{display:inline-block;font-size:13px}}.trader-profile-muted{{color:#C8C8CD;font-size:10px;overflow-wrap:anywhere}}.trader-profile-label{{color:#C8C8CD;font-size:9px;font-weight:700;letter-spacing:1.5px;margin-top:6px}}.trader-wallet-row{{display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:6px 8px;border:1px solid #3a3a40;border-left:2px solid #FF003A;background:#090909}}.trader-wallet-copy{{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:12px;width:100%;background:transparent;border:0;color:#FFF;padding:2px;font:inherit;cursor:pointer;text-align:left}}.trader-wallet-copy:hover{{color:#FFF}}.trader-wallet-copy span{{color:#FF003A;margin-left:0}}.trader-wallet-short{{white-space:nowrap;overflow:visible;font-size:10px;letter-spacing:0}}.trader-profile-stats-title{{display:flex;justify-content:space-between;color:#FF003A;font-size:10px;font-weight:700;letter-spacing:1.5px;margin-top:10px;border:1px solid #FF003A;border-bottom:0;padding:6px 8px}}.trader-profile-stats-title::after{{content:"ONCHAIN PERFORMANCE";color:#77777d;font-weight:400}}.trader-profile-ranks{{border:1px solid #303035;margin-top:0;padding:0;font-size:10px;background:#080808}}.trader-profile-ranks div{{display:grid;grid-template-columns:12px minmax(70px,1fr) 44px minmax(100px,1fr);gap:6px;align-items:center;border:0;border-bottom:1px solid #303035;background:#0b0b0b;padding:4px 6px;margin:0;min-height:18px}}.trader-profile-ranks div:last-child{{border-bottom:0}}.trader-profile-ranks div::before{{content:"//";color:#FF003A}}.trader-profile-ranks b{{color:#FFF;border-right:1px solid #FF003A;padding-right:6px}}.trader-profile-ranks em{{color:#C8C8CD;font-style:normal;text-align:right;overflow-wrap:anywhere}}@media (max-width:768px){{.trader-profile-card{{--trader-profile-square:auto;width:min(560px,calc(100vw - 24px));left:0}}.trader-profile-card-grid{{grid-template-columns:1fr;height:auto}}.trader-profile-avatar{{width:min(100%,300px);height:auto;aspect-ratio:1/1}}.trader-profile-content{{height:auto;max-height:none}}.trader-profile-identity strong{{font-size:24px}}.trader-profile-content::before{{font-size:8px}}}}</style><div class="trader-table-scroll"><table class="trader-table"><thead><tr>{''.join(f'<th>{c}</th>' for c in columns)}</tr></thead><tbody>{''.join(body)}</tbody></table></div>"""
    css = _trader_profile_card_styles(metric_icons) + css
    css += f'<style>.trader-table .trades-value{{color:{TRADES_COLOR};font-weight:700}}</style>'
...
def _render_metric_guide() -> None:
    ...
    render_section_guide_panel("""<p><b>TOP TRADERS ANALYTICS</b> Compare wallets by observed OpenSea trading activity and realized trading performance.</p><p><b>FILTERS</b> Select or enter a wallet address to locate a specific trader. The leaderboard automatically moves to the page containing that wallet. On desktop, the matching table row is highlighted while the surrounding ranking context remains visible.</p><p><b>SORT BY</b> EARNED ranks by realized profit/loss. INVESTED ranks by total observed purchase value. SOLD ranks by total observed sale value. TRADES ranks by total observed marketplace participations.</p><p><b>TRADER PROFILE</b> On desktop, hover over a trader's IMAGE to open the detailed Trader Profile Card. Clicking the trader name opens the trader's OpenSea profile. On mobile, TOP TRADERS is displayed directly as standalone Trader Profile Cards and no hover is required.</p><p><b>PROFILE CARD</b> The card includes the OpenSea profile link, copyable wallet address and the trader's rank and value for EARNED, INVESTED, SOLD, TRADES, ROI and WIN RATE. ROI and WIN RATE are Profile Card ranks only; they are not sidebar SORT BY options.</p><p><b>EARNED</b> Realized profit/loss only from sales that can be matched to a previous purchase of the exact same NFT. <b>INVESTED</b> is the total value of all observed purchases. <b>SOLD</b> is the total value of all observed sales.</p><p><b>ROI</b> Realized return on the acquisition cost of matched sold NFTs only. It is not return on total Invested.</p><p><b>WIN RATE</b> The share of evaluable matched sales closed in profit.</p><p><b>DESKTOP TABLE</b> COVERAGE is the share of observed sales that could be matched to a previous acquisition. MATCHED SALES are sales linked to an observed previous purchase of the exact contract + tokenId. These fields are shown in the desktop table; the Profile Card focuses on the six ranked performance metrics.</p><p class="trader-guide-note"><b>WIN RATE PROFILE RANK</b> requires supported P&amp;L data, at least 3 matched sales and at least 50% sale coverage. When those conditions are not met, the Win Rate rank in the Profile Card may be unavailable.</p><p class="trader-guide-note"><b>CURRENCY</b> With USD PRICE enabled, monetary metrics, ROI and Win Rate use historical USD values based on the GUN/USD price at the time of each transaction. With USD PRICE disabled, those metrics use GUN values.</p><p class="trader-guide-note">Metrics are based only on observed OpenSea activity. Unknown external transfers, mints, in-game acquisitions, other marketplaces, fees and royalties are not included unless explicitly present in the source data.</p>""".replace("Select or enter a wallet address", "Search by trader name or wallet address"), trusted_html=True)
...
def render_trader_mobile_cards(rows: list[dict[str, Any]], show_usd: bool = True) -> None:
    ...
    st.markdown(_trader_profile_card_styles(metric_icons) + '<style>.trader-mobile-card-grid{display:grid;grid-template-columns:1fr;gap:16px;width:100%}.trader-mobile-card-grid .trader-profile-card{display:block;position:relative;left:auto;right:auto;top:auto;bottom:auto;width:100%;pointer-events:auto}</style><div class="trader-mobile-card-grid">' + ''.join(cards) + '</div>', unsafe_allow_html=True)
    _render_trader_clipboard_wiring()
