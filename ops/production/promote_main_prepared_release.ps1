...
try {
    ...
    if($Simulation){
        ...
        $manifest=Get-Content $PreparedReleaseManifest -Raw|ConvertFrom-Json;if((Hash $PreparedReleaseManifest) -ne $PreparedReleaseManifestSha256.ToLowerInvariant()){throw 'PREPARED_RELEASE_MANIFEST_HASH_MISMATCH'};if($manifest.prepared_release_head -ne $PreparedReleaseHead){throw 'PREPARED_RELEASE_HEAD_MISMATCH'};if($state.main_head -ne $ExpectedMainHead){throw 'EXPECTED_MAIN_HEAD_MISMATCH'}
    ...
    if((Hash $PreparedReleaseManifest) -ne $PreparedReleaseManifestSha256.ToLowerInvariant()){throw 'PREPARED_RELEASE_MANIFEST_HASH_MISMATCH'};$manifest=Get-Content $PreparedReleaseManifest -Raw|ConvertFrom-Json;if($manifest.prepared_release_head -ne $PreparedReleaseHead){throw 'PREPARED_RELEASE_HEAD_MISMATCH'};& git -C $repo merge-base --is-ancestor $ExpectedMainHead $PreparedReleaseHead;if($LASTEXITCODE -ne 0){throw 'PROMOTION_NOT_FAST_FORWARD'}
    if(-not$Execute){KV 'MODE' 'DRY_RUN';KV 'PROMOTION_PLAN' 'PASS';KV 'EXACT_PUSH_REF' ($PreparedReleaseHead+':refs/heads/main');KV 'DEVELOP_BRANCH_PROMOTION_FORBIDDEN' 'YES';KV 'MUTATION_EXECUTED' 'NO';exit 0}
