...
try {
    ...
    $schema=Read-SchemaState $dbEnvironment $tools.psql;if($schema.ReadOnly -ne 'on'){throw 'DB_NOT_READ_ONLY'};if($schema.StableBrowserIdentity -ne 'READY' -or $schema.TraderMode -ne 'ABSENT' -or $schema.ProductEvents -ne 'ABSENT' -or $schema.UserFeedback -ne 'ABSENT'){throw 'DB_SCHEMA_DRIFT'}
